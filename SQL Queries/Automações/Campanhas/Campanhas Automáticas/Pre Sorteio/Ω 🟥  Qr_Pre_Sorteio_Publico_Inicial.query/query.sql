...
FROM (
  SELECT
    ...
    D.HC_NUMERO_DA_SORTE__C,
    SELECTED.PREMIO_EMAIL,
    SELECTED.DATA_SORTEIO,
  ...
  FROM (
    SELECT
      ...
      C.ID AS CONTACTID,
      D.ID AS TITULOID,
      PLANOS.PREMIO_EMAIL AS PREMIO_EMAIL,
      format(dateadd(dd, [Dispara dias antes], getdate()), 'dd/MM/yyyy') as DATA_SORTEIO,
    ...
    where 
      ...
      AND (
        ...
        /* ou pode receber sms */
        /* A Jornada está sem SMS, por enquanto não checa se tem celular ou não */
      ...
      AND format(dateadd(dd, [Dispara dias antes], getdate()), 'dddd') = [Dia da semana]
        /* mes da execucao */
      AND (
        (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 1 AND JANEIRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 2 AND FEVEREIRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 3 AND MARCO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 4 AND ABRIL = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 5 AND MAIO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 6 AND JUNHO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 7 AND JULHO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 8 AND AGOSTO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 9 AND SETEMBRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 10 AND OUTUBRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 11 AND NOVEMBRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], getdate())) = 12 AND DEZEMBRO = 1)
      )
      AND (
        ...
        OR (
          ...
          AND DATEPART(dd, dateadd(dd, [Dispara dias antes], getdate())) <= 7
        )
      )
    ) SELECTED

...
FROM
    (
    	SELECT
        	...
        	c.HC_Data_da_Ultima_Contemplacao__c AS Data_ultima_contemplacao,
    	...
    	WHERE
        	...
        	AND
        	c.HC_Data_da_Ultima_Contemplacao__c BETWEEN DATEADD(DD, 21, EOMONTH(GETDATE(),-3)) AND DATEADD(DD, 20, EOMONTH(GETDATE(),-2))
    ) sub
WHERE
    sub.ranking = 1
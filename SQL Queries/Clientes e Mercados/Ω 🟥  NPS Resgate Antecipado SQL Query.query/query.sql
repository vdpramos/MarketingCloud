SELECT TOP 300000
	...
	A.HC_CANAL_RESGATE_OTC__c AS CodigoOTC,
	A.HC_Data_do_ultimo_resgate__c AS DataUltimoResgate
FROM HC_Titulo__c_Salesforce A
LEFT JOIN HC_Relacao_Cliente_Parceiro__c_Salesforce B ON B.HC_Cliente__c = A.HC_Cliente__c
WHERE
    A.HC_Familia_do_produto_do_titulo__c = 'Ourocap'
AND B.HC_Tipo_de_Pessoa__c = 'PF'
AND A.HC_Situacao__c IN ('Finalizado - Antecipado total', 'Finalizado - Resgate Pago', 'Baixado Resgate Pago')
AND (B.HC_Id_Parceiro_Cap__c = 'TCBB' OR B.HC_Id_Parceiro_Darwin__c = '1')
AND B.HC_Classificacao__c = 'Ex-cliente'
AND A.HC_Data_do_ultimo_resgate__c < '2023-07-01'
ORDER BY
    A.HC_Data_do_ultimo_resgate__c DESC
SELECT
    ...
    c.Id AS Id_atendimento,
    c.CreatedDate AS Data_abertura
FROM Close_the_loop_clientes_ativos a
    ...
    INNER JOIN Case_Salesforce c
        ON b.Id = c.HC_Relacao_Cliente_x_Parceiro__c
WHERE
    ...
    AND
    c.RecordTypeId = '0126g000000XFHwAAO'
    AND
    c.CreatedDate BETWEEN DATEADD(DD, 21, EOMONTH(GETDATE(),-2)) AND DATEADD(DD, 20, EOMONTH(GETDATE(),-1))
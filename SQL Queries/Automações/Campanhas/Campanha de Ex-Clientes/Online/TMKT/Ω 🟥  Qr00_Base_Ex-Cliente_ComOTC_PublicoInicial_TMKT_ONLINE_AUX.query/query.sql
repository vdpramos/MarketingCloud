...
FROM (
    ...
    LEFT JOIN TB_BASE_QUARENTENA Q ON Q.CPF_CNPJ = B.HC_CPF_CNPJ__C 
      AND Q.DATA_PARTICIPACAO_CAMPANHA > GETDATE()
    /* Remove ja adicionados */
    LEFT JOIN Publico_Apto_Ex_Clientes_TMKT_ONLINE aptos ON aptos.CPF_CNPJ = B.HC_CPF_CNPJ__C
    LEFT JOIN Publico_Inapto_Ex_Clientes_TMKT_ONLINE inaptos ON inaptos.CPF_CNPJ = B.HC_CPF_CNPJ__C
    WHERE
        aptos.CPF_CNPJ IS NULL /* Não inserir contatos já adicionados */
      AND inaptos.CPF_CNPJ IS NULL
      AND A.[TYPE] = 'Ex-Cliente'
      /* AND B.HC_RNA__c = '000' */
			/* Remove quarentena */

SELECT DISTINCT
     replace( replace( replace( replace( replace( replace( replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (replace (A.NOME, 'á', 'a'),'é', 'e'),'í', 'i'),'ó', 'o'),'ú', 'u'),'Á', 'A'),'É', 'E'),'Í', 'I'),'Ó', 'O'),'Ú', 'U'),'à', 'a'),'À', 'A'),'ã', 'a'),'õ', 'o'),'Ã', 'A'),  'Õ', 'O'),'â', 'a'),'ê', 'e'),'ô', 'o'),'Â', 'A'),'Ê', 'E'),'Ô', 'O'),'ç', 'c'),'Ç', 'c') , '-', '') , '+', '') , '|', '') , '$', '') , '%', '') , '&', '') as NOME,
...
FROM Tb_Base_Inicial_Prospect_ComOTC_Online_AeC A
...
WHERE CA.HC_FORNECEDOR__C = 'AeC'
AND CA.[TYPE] = 'Prospect'
AND CA.IsActive = 1
AND CA.Id = '7016g000000FhMSAA0'
AND Q.CPF_CNPJ IS NULL
AND (
	   (LEN(NULLIF(TEL_01, '')) < 10 AND (( LEN(TEL_01) = '9' AND TEL_01 LIKE '9%' ) OR LEN(TEL_01) = '8' AND TEL_01 NOT LIKE '9%' ))
	OR (LEN(NULLIF(TEL_02, '')) < 10 AND (( LEN(TEL_02) = '9' AND TEL_02 LIKE '9%' ) OR LEN(TEL_01) = '8' AND TEL_01 NOT LIKE '9%' ))
	OR (LEN(NULLIF(TEL_03, '')) < 10 AND (( LEN(TEL_03) = '9' AND TEL_03 LIKE '9%' ) OR LEN(TEL_01) = '8' AND TEL_01 NOT LIKE '9%' ))
	OR (LEN(NULLIF(TEL_04, '')) < 10 AND (( LEN(TEL_04) = '9' AND TEL_04 LIKE '9%' ) OR LEN(TEL_01) = '8' AND TEL_01 NOT LIKE '9%' ))
	OR (LEN(NULLIF(TEL_05, '')) < 10 AND (( LEN(TEL_05) = '9' AND TEL_05 LIKE '9%' ) OR LEN(TEL_01) = '8' AND TEL_01 NOT LIKE '9%' ))
)

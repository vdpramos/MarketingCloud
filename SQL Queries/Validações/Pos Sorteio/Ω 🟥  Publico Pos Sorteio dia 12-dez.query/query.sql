...
FROM (
  SELECT
    ...
    Premio_Email,
    Descricao,
    format(convert(datetime, '2022-12-12'), 'dd/MM/yyyy', 'pt-br') as Data_Envio,
    format(dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12')), 'dd/MM/yyyy', 'pt-br') as Data_Sorteio,
  ...
  FROM (
    ...
    where 
      ...
      A.ID = '7016g000000TdvVAAS'
          /* Data de Execução */
      /* AND CONVERT(VARCHAR(10), A.HC_DATA_DE_EXECUCAO__C, 101) = CONVERT(VARCHAR(10), convert(datetime, '2022-12-12'), 101) */
      /* AND DATEPART(HH, A.HC_DATA_DE_EXECUCAO__C) = DATEPART(HH, convert(datetime, '2022-12-12')) */
      ...
      AND format(dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12')), 'dddd') = [Dia da semana]
        /* mes da execucao */
      AND (
        (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 1 AND JANEIRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 2 AND FEVEREIRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 3 AND MARCO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 4 AND ABRIL = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 5 AND MAIO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 6 AND JUNHO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 7 AND JULHO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 8 AND AGOSTO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 9 AND SETEMBRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 10 AND OUTUBRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 11 AND NOVEMBRO = 1)
        OR (DATEPART(month, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) = 12 AND DEZEMBRO = 1)
      )
      AND (
        ...
        OR (
          ...
          AND DATEPART(dd, dateadd(dd, [Dispara dias antes], convert(datetime, '2022-12-12'))) <= 7
        )
      )
    ) SELECTED
